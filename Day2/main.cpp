...
int main() {
    ...
    while(input_file >> input1 >> input2) {
        switch (opponent_plays(input1[0])) {
            ...
            switch(we_play(input2[0])) {
                case we_play::ROCK: total_score += 1 + 0; continue;
                case we_play::PAPER: total_score += 2 + 3; continue;
                case we_play::SCISSORS: total_score += 3 + 6; continue;
            }
            case opponent_plays::SCISSORS:
            switch(we_play(input2[0])) {
                case we_play::ROCK: total_score += 1 + 6; continue;
                case we_play::PAPER: total_score += 2 + 0; continue;
                case we_play::SCISSORS: total_score += 3 + 3; continue;
            }
        }
    }
